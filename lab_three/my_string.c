...
Status my_string_extraction(MY_STRING hMyString, FILE* fp);
...
MY_STRING my_string_init_default(void)
{
	...
	if (pMy_string != NULL)
	{
		...
		{
			printf("failed to allocate in init_default\n");
			free(pMy_string);
			return NULL;
		}
...
Status my_string_extraction(MY_STRING hMy_string, FILE* fp)
{
	...
	while (noc == 1 && c != ' ' && c != '\n')
	{
		...
		{
			int newSize = (pString->capacity * 2);
